 ...
 * 1. Export all variables from all environments (decrypted)
 * 2. Generate a new encryption key
 * 3. Backup the old key
 * 4. Re-import all variables (encrypted with new key)
 */
async function runKeyRotate(context: KeyContext): Promise<void> {
  ...
  const project = getProjectName(context)
